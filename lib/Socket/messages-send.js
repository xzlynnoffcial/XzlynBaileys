...
const XzlynHandler = require('./dugong');
...
const makeMessagesSocket = (config) => {
    ...
    const xzlyn = new XzlynHandler(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = xzlyn.detectType(content);
            if (typeof content === 'object' && 'disappearingMessagesInChat' in content &&
                ...
                if (messageType) {
                    switch (messageType) {
                        case 'PAYMENT':
                            const paymentContent = await xzlyn.handlePayment(content, quoted);
                        ...
                        case 'PRODUCT':
                            const productContent = await xzlyn.handleProduct(content, jid, quoted);
                        ...
                        case 'INTERACTIVE':
                            const interactiveContent = await xzlyn.handleInteractive(content, jid, quoted);
                        ...
                        case 'ALBUM':
                            return await xzlyn.handleAlbum(content, jid, quoted)
                        case 'EVENT':
                            return await xzlyn.handleEvent(content, jid, quoted)
                        case 'POLL_RESULT':
                            return await xzlyn.handlePollResult(content, jid, quoted)
                        case 'GROUP_STORY':
                            return await xzlyn.handleGroupStory(content, jid, quoted)
                    }
                }
